 ...
 *
 * Language files are fetched from the mainul94/books repo
...
async function fetchContentsFromApi(code: string) {
  const url = `https://api.github.com/repos/mainul94/books/contents/translations/${code}.csv`;
...
async function fetchContentsFromRaw(code: string) {
  const url = `https://raw.githubusercontent.com/mainul94/books/master/translations/${code}.csv`;
  const res = await errorHandledFetch(url);
  if (res === null || res.status !== 200) {
    return null;
  }
...
async function getLastUpdated(code: string): Promise<Date> {
  const url = `https://api.github.com/repos/mainul94/books/commits?path=translations%2F${code}.csv&page=1&per_page=1`;
  const res = await errorHandledFetch(url);
  if (res === null || res.status !== 200) {
