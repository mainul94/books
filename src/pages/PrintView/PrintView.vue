<template>
  <div class="flex flex-col flex-1 bg-gray-25 dark:bg-gray-875">
    <PageHeader :border="true" :title="t`Print View`">
      <AutoComplete v-if="templateList.length" :df="{
    ...
    <div class="overflow-auto custom-scroll custom-scroll-thumb1 p-4">
      <!-- Display Hints -->
      <div v-if="helperMessage" class="text-sm text-gray-700 dark:text-gray-300">
        {{ helperMessage }}
      </div>

      <!-- Template Container -->
      <PrintContainer v-if="printProps" ref="printContainer" :print-schema-name="schemaName"
        :template="printProps.template" :values="printProps.values" :scale="scale" :width="templateDoc?.width"
...
export default defineComponent({
  ...
  methods: {
    ...
    printTheDoc() {
      ...
      newWindow.document.write(printContainer);
